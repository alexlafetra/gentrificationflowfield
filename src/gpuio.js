...
let particleAgeState,particleDataState,flowFieldState,flowMagState,initialState,particleCanvas,noiseShader;


function initGPUIO(settings){
    canvas = document.getElementById("defaultCanvas0");
    composer = new GPUComposer({canvas});

    //storing particle age
    particleAgeState = new GPULayer(composer, {
		name: 'age',
		dimensions: [dataTextureDimension,dataTextureDimension],
		type: 'FLOAT',
		filter: 'LINEAR',
		numComponents: 1,
		wrapX: 'REPEAT',
    ...
    particleDataState = new GPULayer(composer, {
		name: 'data',
		dimensions: [dataTextureDimension,dataTextureDimension],
		type: 'FLOAT',
		filter: 'LINEAR',
    ...
    //storing flow field
    flowFieldState = new GPULayer(composer, {
		name: 'flow',
		dimensions: [settings.canvasSize,settings.canvasSize],
		type: 'FLOAT',
		filter: 'LINEAR',
		numComponents: 4,
		wrapX: 'REPEAT',
    ...
    //storing flow field
    flowMagState = new GPULayer(composer, {
        name: 'flow',
        dimensions: [settings.canvasSize,settings.canvasSize],
        type: 'FLOAT',
        filter: 'LINEAR',
    ...
    initialState = new GPULayer(composer, {
		name: 'data',
		dimensions: [dataTextureDimension,dataTextureDimension],
		type: 'FLOAT',
		filter: 'LINEAR',
    ...
    //render target for particles
    particleCanvas = new GPULayer(composer, {
        name: 'data',
        dimensions: [dataTextureDimension,dataTextureDimension],
        type: 'FLOAT',
        filter: 'LINEAR',
    ...
    //shader for the noise
    noiseShader = new GPUProgram(composer, {
		name: 'noiseShader',
		fragmentShader: `
		in vec2 v_uv;
		...
		void main() {
			out_state = fract(sin(dot(v_uv.xy,vec2(22.9898-uRandomSeed,78.233+uRandomSeed)))*43758.5453123*uRandomSeed);;
		}`,
		uniforms: [
			{
				name: 'uRandomSeed',
...
function main({ pane, contextID, glslVersion}) {
	...
	const particleState = new GPULayer(composer, {
		name: 'particleState',
		dimensions:[dataTextureDimension,dataTextureDimension],
		type: FLOAT,
		filter:LINEAR,
	...
	const flowFieldTexture = new GPULayer(composer, {
		name: 'flowFieldTexture',
	...
	const particleMaskTexture = new GPULayer(composer, {
	...
	const updateParticleState = new GPUProgram(composer, {
		...
		fragmentShader: glsl`
			...
			uniform sampler2D uParticleAge;
			...
			void main(){
				vec2 vParticleCoord = v_uv;
				//getting the particle data
				vec4 particleData =  texture2D(uParticleState,vParticleCoord);
				...
				vec4 Age = texture2D(uParticleAge,vParticleCoord);
				if(Age.x >= uAgeLimit){
				...
				vec4 flowForce =  texture2D(uFlowField,screenPosition);
		...
		uniforms: [
			{
				name: 'uFlowField',
				value: 0,//what does setting this do?
				type: FLOAT
			},
			{
				name: 'uParticleState',
	...
	function loop() {
		...
		//loop should be:
		/*
		updateParticleData() (velocity and position)
		updateParticleAge()
		renderParticles()
		*/
		// Advect the velocity vector field.
		composer.step({
			program:updateParticleState,
			input:[flowFieldTexture,particleState,particleAgeState,particleMaskTexture,initialState,0.01,0,0,1,1.0,false,false],
